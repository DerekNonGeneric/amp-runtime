(self.AMP=self.AMP||[]).push({n:"amp-springboard-player",v:"1499663230322",f:(function(AMP){(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
/**
...
var AmpSpringboardPlayer = (function (_AMP$BaseElement) {
  babelHelpers.inherits(AmpSpringboardPlayer, _AMP$BaseElement);

  /** @param {!AmpElement} element */

  function AmpSpringboardPlayer(element) {
    babelHelpers.classCallCheck(this, AmpSpringboardPlayer);
  ...
  AmpSpringboardPlayer.prototype.preconnectCallback = function preconnectCallback(opt_onLayout) {
  ...
  AmpSpringboardPlayer.prototype.isLayoutSupported = function isLayoutSupported(layout) {
    return _srcLayout.isLayoutSizeDefined(layout);
  };

  /** @override */

  AmpSpringboardPlayer.prototype.buildCallback = function buildCallback() {
    this.mode_ = _srcLog.user().assert(this.element.getAttribute('data-mode'), 'The data-mode attribute is required for <amp-springboard-player> %s', this.element);
    this.contentId_ = _srcLog.user().assert(this.element.getAttribute('data-content-id'), 'The data-content-id attribute is required for' + '<amp-springboard-player> %s', this.element);
    this.domain_ = _srcLog.user().assert(this.element.getAttribute('data-domain'), 'The data-domain attribute is required for <amp-springboard-player> %s', this.element);
  };

  /** @override */

  AmpSpringboardPlayer.prototype.layoutCallback = function layoutCallback() {
    var iframe = this.element.ownerDocument.createElement('iframe');
    var siteId = _srcLog.user().assert(this.element.getAttribute('data-site-id'), 'The data-site-id attribute is required for' + '<amp-springboard-player> %s', this.element);
    var playerId = _srcLog.user().assert(this.element.getAttribute('data-player-id'), 'The data-player-id attribute is required for' + '<amp-springboard-player> %s', this.element);
    var items = this.element.getAttribute('data-items') || '10';

    iframe.setAttribute('frameborder', '0');
    iframe.setAttribute('allowfullscreen', 'true');
    iframe.id = playerId + '_' + this.contentId_;
    iframe.src = 'https://cms.springboardplatform.com/embed_iframe/' + encodeURIComponent(siteId) + '/' + encodeURIComponent(this.mode_) + '/' + encodeURIComponent(this.contentId_) + '/' + encodeURIComponent(playerId) + '/' + encodeURIComponent(this.domain_) + '/' + encodeURIComponent(items);
  ...
  AmpSpringboardPlayer.prototype.pauseCallback = function pauseCallback() {
  ...
  AmpSpringboardPlayer.prototype.createPlaceholderCallback = function createPlaceholderCallback() {
    var placeholder = this.win.document.createElement('amp-img');

    placeholder.setAttribute('src', 'https://www.springboardplatform.com/storage/' + encodeURIComponent(this.domain_) + '/snapshots/' + encodeURIComponent(this.contentId_) + '.jpg');
    /** Show default image for playlist */
    if (this.mode_ == 'playlist') {
      placeholder.setAttribute('src', 'https://www.springboardplatform.com/storage/default/' + 'snapshots/default_snapshot.png');
    }
...
//# sourceMappingURL=amp-springboard-player-0.1.max.js.map